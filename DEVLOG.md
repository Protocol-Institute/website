...
## Session 44: Track II Parallel Schedule; Program Tab Routing; Special-Session Cleanup

*2026-08-18*

**Tracks:** content, static-site

- Session 43's paused plan to cluster the Wed–Fri General-session talks into named, coded sub-sessions (`I-A`, `I-B`, …) was abandoned rather than revisited — the schedule keeps the plain continuous day-grouped chronological rendering it already had, and the never-committed draft mock (`_draft-schedule.html`) was deleted. In its place, the 8 General-session talks that had no slot (Aayush Sharma, Amber Case, Jim Fournier, Mike Travers, Nicholas Fett, Nicolás Madoery, Stanislav Lvovsky, Surya Kramer) were scheduled as a second track, in 30-minute slots from 19:00 UTC, running in parallel with the two 2.5h “afternoon” special sessions: Worldbuilding in New Nature (Thursday) and The Art of Memory (Friday), 4 talks each. Migration 032 adds `schedule_track` to `symposium_proposals` — `'ii'` marks a parallel-track item, `NULL` is normal single-track scheduling. Kept deliberately distinct from the pre-existing `track` column, which is an unrelated topical SIG/track category on the submission form; reusing that name would have collided semantically and in the data. None of the 8 speakers confirmed these specific times — Venkat had originally planned individual outreach; that follow-up is still open.

- Client-side only, no schema beyond the `schedule_track` column: `buildTrack2Map()` groups Track II items by which special session's date/time block they fall inside (matched against `symposium_sessions.date/start_time/end_time`, already fetched for the special-session badges/links). `withParallelBlocks()` collapses a special session's own items plus its paired Track II items into one placeholder in the day's sorted list, rendered by `renderParallelBlock()` as a small tab widget — two buttons (the special session's name, and “Track II”), one visible panel at a time, no side-by-side columns (ruled out early as unusable on mobile). A “Two-track portion ends” divider (horizontal rule with centered label) closes out each widget so the return to a single continuous track reads clearly, and Track II cards get a neutral gray (`proposal-card--track2`, `#EFEDE9`) distinct from both the default card and the four special-session pastel colors.

- The three-way All/Talks/Workshops tab switcher (built Session 42) had no per-tab URLs — every view lived at the same address. “All” is now suppressed from the tab bar (Talks is the default view) but still renders at `/all` for direct navigation; Talks and Workshops became real, shareable routes at `/talks` and `/workshops`. Implemented via `_redirects` 200-status rewrites back to the same `index.html`, with the page reading `location.pathname` on load to pick the default tab and `history.pushState` keeping in-tab clicks fast (no full reload); a `popstate` listener handles back/forward. Hit a real Cloudflare Pages quirk along the way: pointing the rewrite's destination at the literal `.../index.html` filename chained into CF Pages' own clean-URL canonicalization, which 308-redirected the rewritten request straight back to the bare symposium URL instead of serving content in place — fixed by pointing the rewrite at the directory's clean URL (`/events/protocol-symposium-2026/`) instead, verified live on the deployed site before moving on. Register button now reads “Register (coming soon)” and is visually disabled (gray, non-interactive) until a real registration flow exists.

- Not a regression from any single change, but one this session's own tab-routing work made visible: the All/Talks/Workshops panels all rendered on every page load regardless of which tab was active, and since a proposal can appear in more than one panel (every talk is in both All and Talks, for instance), each proposal's DOM ids (`p-&lt;id&gt;`, `abs-&lt;id&gt;`, comment and edit-form ids, and the Track II widget's `par-&lt;slug&gt;-a/-b` ids) were duplicated across panels. `getElementById` always resolves to whichever copy comes first in the DOM — usually a hidden one — so clicking “Show description,” opening comments, editing, or switching to the Track II tab silently mutated content the user couldn't see, while the visible copy never changed. Root-caused via a user report (“clicking to expand the extended description doesn't work”) that turned out to also explain an earlier-reported symptom (“Track II shows random other talks”) — same bug, two visible effects. Fixed by rendering only the currently active tab's panel, re-rendered on each switch via `activateTab()`; `scrollToHash()` was updated to activate the correct tab first (by looking up the hashed proposal's `type`) so a `#p-&lt;id&gt;` deep link to a workshop still resolves even though Talks is now the default tab.

- Liz Maher withdrew her Worldbuilding in New Nature talk (“Myth-Making in New Nature”) — deleted the proposal and, because D1 enforces a foreign key from `symposium_votes` to `symposium_proposals`, its 21 associated votes first (both backed up to `backups/` before deletion); no other schedule change was needed since it's a shared-block special-session item, not an individually timed slot. That left three of the four special sessions with unfilled block time (Inventing Psychohistory, Worldbuilding in New Nature, Southeast Asia — The Art of Memory was already fully booked): estimated the surplus using the same 30-minute-per-talk convention as the General grid and Track II, then added one “&lt;Session&gt;: Discussion” placeholder item to each, sized to that surplus and scheduled at the *end* of the block (e.g. Psychohistory's discussion sits at 21:00–21:30, after its three real talks) rather than spanning the whole block. Also gave every previously-untimed real talk in Worldbuilding (3) and Southeast Asia (1) an explicit 30-minute `scheduled_time_utc`/`scheduled_end_time_utc` slot — they'd been relying on the session's shared block-start fallback with no individual order — so each of these three sessions now reads as one clean, gapless sequence ending in its discussion slot, while the page copy still notes the order is host-adjustable. Backfilled `symposium_sessions.owner_email` for the two sessions that had none on file, matching the existing Southeast Asia/Samuel Chua pattern: Aneesh Sathe for Inventing Psychohistory, Sachin Benny for Worldbuilding in New Nature — both pulled from their existing talk submissions in the same sessions, and used as the discussion placeholders' speaker.

---

## Session 45: c3po PR Flow Confirmed Fixed; Symposium Registration Audit; Workshop Host Schema Expanded

*2026-09-01*

**Tracks:** operations, member-directory, static-site

- c3po#2 (drop gitignored `monitoring.html` from `WEBSITE_PATHS`) and c3po#3 (blank-line-leak fix in `_patch_meeting_archive()`) both merged in the c3po repo on 2026-08-17, but hadn't been confirmed to actually produce a website PR yet as of Session 44. This session found and reviewed [website PR #7](https://github.com/Protocol-Institute/website/pull/7): opened 2026-08-21, self-updating each 30-minute daemon cycle through 2026-08-28 (the daemon regenerates the branch in place rather than opening a new PR per cycle), carrying 6 new SIG meeting entries across SIGFPT/SIGPFB/SIGPSY. Verified against the standing c3po-PR checklist — every new `meeting-title` link resolved to a directory added in the same PR, each SIG's “Meeting Archive — N sessions” count matched exactly the number of entries added, no summary was dated less than 7 days out, and `git log` on the touched `sigs/*/index.html` files showed no website-side work since Session 41's manual land (549b9d6) that the PR could have clobbered. Merged clean. This closes out the push-flow bug class that had blocked every automated c3po PR for seven weeks (Sessions 39–44) — worth one more confirmed cycle before fully trusting it unattended.

- A pre-restructure link using the old `.html`-suffixed path (`/symposium-2026.html`, with and without a trailing slash) was 404ing — the existing `_redirects` rule only covered the extensionless `/symposium-2026` form. Added both variants, 301 to `/events/protocol-symposium-2026`/`/events/protocol-symposium-2026/`, matching the existing pattern rather than introducing a new one.

- Cross-referenced a 118-row Luma registrant CSV export (dropped in `inbox/`) against all 61 shortlisted D1 `symposium_proposals` rows. Built the speaker list by unioning `speaker_name`/`co_speakers` (talks) and `organizer_name`/`co_organizer_name` (workshops) across every shortlisted proposal, then merged entries sharing an email or a normalized name (handling suffix noise like “, PhD” and the Humboldt-persona byline that shares Venkat's email) down to 58 unique people. Matched against the registrant list by email first, then by exact name. Result: 39 not found, 32 of them with an email on file — written to `inbox/Luma Guest Import - Not Registered Speakers.csv` in the Luma import-template's column format (name, email, four blank custom-question columns) for outreach; the other 7 are co-speakers with no email recorded in D1 at all, listed separately since they can't be imported. Separately tallied the CSV's workshop-selection field into 5 per-workshop CSVs (`inbox/Workshop Registrants - &lt;slug&gt;.csv`, name+email only) plus a count of 25 registrants who selected no workshop — flagged to Venkat that the form actually has 5 workshop options, not the 4 he referenced, since it matches all 5 real `symposium_proposals` workshop rows exactly.

- Requested when approving a new member (Eleonora Brizi) as a workshop host: the target workshop (“Beyond the Artwork: Designing Protocols as Art”, proposal id 75) already had both of its host slots filled (`organizer_name`: Primavera De Filippi, `co_organizer_name`: Anika Meier), and Venkat wanted her added as a third host rather than overwriting either existing one. Rather than a one-off overwrite or a cramped comma-joined name field, added `host3_name/email/bio` through `host5_name/email/bio` to `symposium_proposals` (migration 033, D1 backed up to `backups/` first), mirroring the existing `organizer`/`co_organizer` name+email+bio triple so the pattern generalizes instead of special-casing a third slot. Updated every place that pattern was hardcoded to exactly two: the admin PATCH allowlist (`functions/api/symposium/proposals/[id].js`), the admin edit form (`edit-proposal.html`, three new host sections), each of the 5 static workshop detail pages' client-side `renderProposal()` (organizer list now pushes host3–5 if present), and the program-listing card byline in the merged symposium page (previously hardcoded to `organizer &amp; co_organizer` only, now joins up to 5 names). Eleonora set as host 3 on proposal 75; her member approval and this schema change both landed directly via D1 (see next item) since the admin panel session wasn't available.
...
## Session 47: SIG links in D1, PRG onboarding, symposium schedule moves

*2026-09-08*

**Tracks:** static-site, content, operations

- SIGs had no structured link storage. A group's own website or any related resource had to be hand-written into `sigs/&lt;slug&gt;/index.html`, which meant it drifted with no mechanism to notice: MRG and SIGFPT displayed no affiliated project at all, while ProtFiSIG, SIGPfB and SIGPSY linked local stub pages instead of the live domains already recorded in `projects.sig_slug`. The new table holds links that are *not* projects — `kind='website'` renders as a labelled line under the page blurb, `kind='link'` in a resources block. **Affiliated projects are deliberately not duplicated into it.** `projects.sig_slug` remains the single source of truth and the SIG pages read it from `/api/projects?sig=` at render time. Overloading `projects` to also hold SIG homepages was considered and rejected: a project row requires `lead_slug`, `description`, `type`, `artifact_type` and `submitted_by`, receives a value score, appears in `/research`, and accepts votes, team-join requests and challenge links. A SIG homepage has none of those properties, so filing one as a project would have meant fabricating five required fields and then adding a suppression flag to keep it out of the research directory — the point at which the overload costs more than a second table. This closes off the alternative: link-shaped SIG data now has a home that is not the projects table.

- Each `sigs/&lt;slug&gt;/index.html` now carries two placeholders — `#sig-website` inside `.about-body` and `#sig-resources` above the Meeting Archive heading — filled by a renderer appended to `js/main.js` that runs only on a path matching `^/sigs/&lt;slug&gt;$`. The four hand-written `.sig-projects` blocks were deleted. Consequence for future work: **do not hand-write link markup into a SIG page**; it will be correct once and then silently wrong, which is exactly the failure this replaced. A SIG page newly generated by c3po's full-render path will lack both placeholders and needs them added by hand — c3po's template has no knowledge of them.

- `_patch_meeting_archive` (c3po `ingest/generate_sig_pages.py`) replaces from the Meeting Archive `&lt;h2 class="section-label"&gt;` through to the `sig-cta` div, and full-renders only when `index.html` is absent — so the placeholders are outside what it rewrites and survive its patches. But the new `&lt;div id="sig-resources"&gt;` sits immediately *above* that h2, and the h2 line itself carries the session count that c3po updates every run. Any c3po PR branched before this change therefore conflicts on exactly those two adjacent lines in whichever SIG pages gained sessions. The resolution is always the same: keep both sides — our div and c3po's updated count. Observed on PR #9 in `sigs/mrg` and `sigs/sigfpt`; disappeared once c3po rebased.

- Adding PRG surfaced that the SIG slug list is duplicated in six places. The functional break was `functions/api/projects.js`, whose `VALID_SIGS` set would have rejected any PRG project submission with 'Invalid SIG'. The other four are display-label maps that would have shown a raw slug: `js/research.js`, `projects/submit/index.html`, `admin/projects.html`, `members/dashboard/index.html`. Worker-side validation now imports `functions/_shared/sigs.js`, so the two API endpoints share one list. **The four client-side copies were left as copies** — they are plain scripts with differing label text per page, and consolidating them onto `window.PI_SIGS` would mean adding a script dependency to four pages for a list that changes roughly annually. Anyone adding an eighth SIG must still touch all five, plus `js/sig-meta.js` and `sigs/CONVENTIONS.md`.

- Four moves: Jim Fournier from Thu Track II 20:00 to Fri Track II 21:00–21:30 (after Nicolás Madoery, inside The Art of Memory's 19:00–21:30 block so the Track II tab still groups him); Kaliya Young's 'Group Brains' to Fri 18:30; 'Artisanal Bots' into the Thursday Track II gap that first move created, at 20:00–20:30; Closing Session shifted 30 minutes earlier to 21:30–23:00 to close the resulting Friday gap. **Updating `scheduled_time_utc` without `scheduled_end_time_utc` produces a visibly broken range** — the first move rendered as '21:00 – 20:30' because the end time was still Thursday's. Note the two columns follow different conventions by track: Track II entries and special-session talks carry explicit end times, while most Track I slots leave `scheduled_end_time_utc` NULL and the renderer shows the start alone. Match the destination's convention, not the origin's.

- Troubleshooting 'SIG meeting notes haven't updated since July' found generation working and delivery stalled. The pipeline is daemon → regenerates pages on disk every 30 min → *weekly-gated* PR → manual merge, so content is only as fresh as the last merged PR. MRG's 2026-07-16 session had been generated and unmerged for seven weeks. **An unmerged c3po PR is indistinguishable from a broken pipeline when viewed from the live site.** PR #9's first head then failed two checklist items: completed summaries for meetings 1 and 4 days old (inside the 7-day grace window), and a SIGPSY session dated 2023-08-13 — impossible, since that group started 2026-06-04 — whose 'summary' described a meeting that had only been scheduled. Per governance these were filed back as c3po#4 and c3po#5 rather than hand-fixed in the PR; c3po fixed both in ~15 minutes. The fix commit confirms the diagnosis: `meeting_ready()` existed and was correct, but the renderer never consulted it. Merged as `1a1b737`, clearing MRG to Aug 27 and DRG/ProtFiSIG to Aug 20/Aug 6.

- After deploying, the new SIG rendering did not appear in-browser while `curl` showed the correct file — the browser was running a cached 11KB `main.js` against a 14KB deployed one. `/js/main.js` carries `cache-control: public, max-age=14400`, so returning visitors run up to four-hour-old JavaScript. This is long-standing behaviour for every JS change on this site, not new; worth remembering before debugging a 'the change isn't live' report, and worth considering a cache-busting query or shorter max-age if client-side rendering keeps growing.

---

## Session 48: c3po's PR is opened by a workflow in this repo, not by the generator

*2026-09-12*

**Tracks:** operations

- c3po's SIG-page generator force-pushes `c3po/auto-sig-pages` here about once a week and used to call `gh pr create` itself. That single API call was the *only* reason the generating host held a GitHub token at all — and the token it held turned out to reach every repository on the account with admin rights, on a box whose two services exist to consume untrusted external input (see `Code/incidents/2026-09-09-c3po-vm-github-token-scope.md`). Deploy keys would have been the clean fix, but `POST /repos/{repo}/keys` returns `422 Deploy keys are disabled` for every Protocol-Institute repo — the policy is org-wide, so that option is closed here regardless of this change. The generator therefore moved to a fine-grained token scoped to `Contents: write` on three repos. Contents-write can push a branch; it cannot open a PR.`.github/workflows/c3po-auto-pr.yml` (PR [#10](https://github.com/Protocol-Institute/website/pull/10), merged `e2272f0`) supplies that missing step from this side, using the run's own `GITHUB_TOKEN` with `contents: read` + `pull-requests: write`. The net effect is that the generating host now has **no GitHub API access at all** — push and nothing more. **The direction this locks in:** capabilities that only exist to serve this repo now live in this repo. Anything c3po later needs beyond pushing a branch should be added as a workflow here rather than as a scope on a credential over there.

- Triggered only by `push` on `c3po/auto-sig-pages`; pushes to `main` or any other branch are untouched. It lists open PRs for that head and exits 0 if one exists — pushing to an open PR's branch already updates it, so the workflow only has to act on the first push after each merge. PRs opened with `GITHUB_TOKEN` do not trigger further workflow runs, and nothing here listens on `pull_request`, so there is no loop.Two things a future debugger needs to know, because both fail quietly:**A push that produces no PR is now a failure in this repo's Actions tab**, not in the generator's log. That is a change of debugging location, and the old habit of reading `daemon.log` first will now look like silence.**A `push` trigger reads workflow files from the pushed ref, not from `main`** — so the workflow only fires if the regenerated branch itself carries the file. It does, because the generator hard-resets to `origin/main` before `checkout -B`. That reset is now load-bearing for PR-opening, not just for keeping the branch current.Review also confirmed the nested `$(cat &lt;&lt;'BODY' … BODY)` heredoc dedents to column 0 under the YAML block scalar (checked with `bash -n`) — the one construct in the file that would break silently at runtime rather than at parse time.

- The generator's `gh pr create` call was removed before this workflow merged, so between the two changes a weekly push would have landed a branch with no PR opened and no error anywhere. That is precisely the shape of the seven-week delivery lag found in Session 47, where three SIGs showed July content on the live site purely because a generated PR sat unmerged. No push fell in the gap — `c3po/auto-sig-pages` was deleted when PR #9 merged and has not been recreated — but the ordering was the risk, and it is now closed.**Still unexercised:** the next weekly push is this workflow's first live test. Until then, treat the flow as untested in production rather than working.

- The governance section had been accumulating chronological bug reports and was actively misleading: its lead still said the PR flow &quot;has not actually been working,&quot; which stopped being true at PR #7 (Session 45). Restructured so current mechanism comes first (a new *Who opens the PR* subsection), with the 2026-07 through Session 41 failures compressed into one *Push-flow history (resolved — do not re-debug)* paragraph. The standing pre-merge review checklist is unchanged and still the operative part. The session-start step that told a reader to interpret dirty `sigs/*/index.html` as the known push bug now points at the fixed flow instead.

---

## Session 49: Luma guest-list reprocessing, and three symposium program changes

*2026-09-15*

**Tracks:** content, operations

- Re-ran the Luma export (2026-09-11, 161 rows) into the five per-workshop CSVs in `inbox/`. Workshop picks went 93 &rarr; 111, entirely by addition &mdash; no one dropped or switched.The load-bearing discovery is that `approval_status: invited` does *not* mean registered: it is an invitation the host sent that the guest has not accepted. Counting raw rows therefore overstates attendance. The correct decomposition of the 161 rows is **148 registered** (146 `approved` + 2 off the cleared waitlist), 8 `invited`-but-unaccepted, 2 `declined`, and 3 duplicate rows. Any future headcount off this export must net those out.The duplicates are a specific artifact of the Session 45 speaker import: three people (Sean Stevenson, Simone Cicero, Surya Kramer) were imported under one email and had separately self-registered under another, leaving a live registration and a stale pending invite for the same person. Deduping keeps the self-registration &mdash; it carries the form response, the invite carries nothing. Written to `inbox/Protocol Symposium 2026 - Guests - 2026-09-11 (deduped).csv` (158 unique people). **The duplicates still exist in Luma itself**; only the local export was cleaned, so those three will receive doubled reminders and check-in QRs.Reconciling against the Session 45 outreach list closed it out: 24 of 32 imported speakers have registered, 0 unaccounted for, 8 still pending. That makes `inbox/Luma Guest Import - Not Registered Speakers.csv` 24/32 stale &mdash; it must be regenerated, not reused.

- Gary Wolf withdrew. The row (id 9) was **not** deleted and **not** overwritten with the replacement session. Instead `is_shortlisted` went to 0 and the schedule fields were nulled.This is the pattern to reuse, for two reasons. First, `/api/symposium/proposals` filters on `WHERE p.is_shortlisted = 1`, so clearing the flag is sufficient to remove a session from the public program &mdash; no deletion is needed for the user-visible outcome. Second, `symposium_votes` rows reference `proposal_id`, and this proposal carried **25 voters' ballots totalling 54 votes**; deleting the row would have destroyed community voting signal that cannot be reconstructed. Overwriting the row in place would have been just as lossy, discarding the original submitted abstract. Re-listing a withdrawn session is now a one-field flip plus restoring the date/time. Pre-change row archived to `backups/proposal-9-gary-wolf-2026-09-15.json`.The vacated slot (Sat 2026-09-24 17:00 UTC, Track I) was filled by a new row, id 106, `open-mic-frontier-pacing-protocols`. It follows the `closing-session` convention for Institute-run sessions rather than the submitted-proposal convention: plain descriptive slug, `venkat@protocol-institute.org`, no `artifact_type`. Typed `interactive` rather than `talk` &mdash; both render identically in the schedule grid (only `workshop` draws a badge), so the type is free to carry accurate semantics.

- The symposium header was restructured now that registration is closed: the "Registration closed" button is demoted to a small chip (`.register-btn--mini`, absorbing the separate "Event at capacity" line) and a prominent **Watch Livestream** button takes the primary slot. It ships as a real `&lt;button disabled&gt;`, not a grey-styled `&lt;span&gt;` &mdash; `disabled` blocks click *and* keyboard activation, so there is no focusable dead control. Swap to `&lt;a class="livestream-btn"&gt;` with an href when the stream URL exists.Below it, a countdown with three phases: before 2026-09-21 12:00Z "Symposium starts in X"; until 2026-09-23 15:00Z "Workshop days in progress. Main symposium starts in X"; until 2026-09-25 22:30Z "Symposium in progress."; then it hides itself via `.countdown:empty`. The last two states were not requested but are necessary &mdash; without them the line would freeze at "starts in 0 minutes" for the duration of the event.Two deliberate structural choices. The countdown lives in **its own IIFE outside the main one**, so it renders even if `/api/symposium/proposals` fails; it is header furniture and must not inherit the program fetch's failure modes. And its two boundaries are **hardcoded UTC instants rather than derived from the proposals feed**: they are fixed event facts, whereas the feed's earliest row moves whenever any talk is rescheduled &mdash; a header that silently re-targets itself when someone edits the program is a worse bug than one that needs a manual edit if the event moves. **If the event dates change, these two constants must be updated by hand.**Note the naming trap: the record with slug `opening-session` ("Welcome Session", id 96) is on the **23rd**, the main-symposium start &mdash; not the 21st, which is workshop days beginning 12:00Z. "The opening session" and "when the event starts" are two different instants here.

- Spencer Nitkey (id 94) and Sachin Benny's "Making Monsters" (id 104) exchanged places in the Worldbuilding in New Nature special session. These are unequal blocks &mdash; 30 min and 60 min &mdash; so "swap them" is genuinely ambiguous: the sessions can carry their durations with them (re-laying the block's internal boundaries) or the slots can stay fixed and the sessions trade them. Venkat chose the latter, so **Making Monsters is now 30 min (was 60) and Creating New Genres 60 (was 30)**, with the block still running 19:00&ndash;21:30 gapless.Mechanically: these are Track I rows but *with explicit* `scheduled_end_time_utc`, because sub-times inside a special session's block are individually timed even though Track I General-grid talks normally leave the end NULL. Both start and end must be written on a move; updating only `scheduled_time_utc` would silently produce an overlapping or negative-length block. Pre-change rows archived to `backups/worldbuilding-slots-2026-09-15.json`.Also worth noting for future schedule edits: the session had *two* Sachin Benny items, so "swap Sachin and Spencer" had two readings that produced materially different programs. Speaker name is not a unique key in this table &mdash; resolve schedule instructions to proposal ids before writing.

---

## Session 50: Workshop timings, a self-activating livestream, and a live PII leak closed

*2026-09-16*

**Tracks:** content, operations, member-directory

- Six rows in `symposium_workshop_sessions`. The Stigmergy Hackathon shifted +2h to 15:00&ndash;16:30 and 20:00&ndash;21:30 UTC on both days, with sessions 1 and 3 also **shortened from 2.5h to 1.5h** &mdash; the UTC figures and the supplied PDT gloss disagreed by an hour on those two, and Venkat confirmed the shorter reading, taking total contact time from 8h to 6h. Robotics sessions 2 and 4 moved 20:30 to 18:30&ndash;19:30; 1 and 3 were already correct. **No code changed:** the detail pages and the program listing render these straight from D1, which is the point of the migration-031 table &mdash; a timing change is a data edit, not a deploy. The limit of that design is everything *outside* D1: the Luma event pages and each workshop's Google Form registration page carry their own copies of the times and were not updated.

- The header's &ldquo;Watch Livestream&rdquo; control had been a permanently disabled `&lt;button&gt;` with no URL. It now ships with `youtube.com/@protocol-institute/live` staged in markup and flips itself on at `LIVESTREAM_GO_LIVE = 2026-09-23T14:45:00Z` &mdash; 15 minutes before the Welcome Session, whose 15:00 UTC start was checked against D1 rather than trusting the existing `SYMPOSIUM_START` constant. This deliberately follows the countdown's precedent of hardcoding fixed event facts as absolute UTC rather than deriving them from the proposals feed, whose earliest row shifts whenever a talk is rescheduled. **Both** the disabled button and the live anchor ship, with JS toggling `[hidden]`: the button is the no-JS default, so a script failure degrades to an honest disabled control rather than a live-looking dead one. `.livestream-btn[hidden]{display:none}` is load-bearing &mdash; the class sets `display:inline-block`, which outranks the `[hidden]` UA rule and would otherwise reveal the staged anchor immediately. It reverts to disabled after `EVENT_END`, because the `/live` URL 404s when nothing is broadcasting. It lives in the countdown IIFE, which is deliberately decoupled from the program fetch, so the CTA survives a failed `/api/symposium/proposals` call.

- The workshop detail pages listed UTC only while the program page had shown a computed viewer-local time; they were brought to parity first. That exposed a defect both pages shared: `localLabel` rendered a local *time* but kept the *UTC date* printed beside it, so a viewer far enough east read &ldquo;Mon, Sep 21 &middot; 20:00 UTC (1:30 AM in India Standard Time)&rdquo; for an instant that is Tuesday where they are. With the program running 12:00&ndash;23:00 UTC this is not an edge case &mdash; every viewer east of roughly UTC+3 hits it on the evening sessions. Fixed on both renderers in one change, since fixing one would have made them disagree. Adopts the airline-itinerary convention, with offsets measured **per end against the UTC date on that line**, never against each other, so a marker always means &ldquo;relative to the date shown here&rdquo;: both ends on the same local day take one marker for the pair, differing ends are each marked. `localDayOffset` compares calendar dates rather than elapsed hours &mdash; both sides parse as UTC midnight, so a DST transition in the viewer's zone cannot make the subtraction land on a fraction and round wrong. Verified by extracting the shipped functions from both files and asserting identical output across 11 zones including Kiritimati (UTC+14), Midway (UTC&minus;11) and both 2026 US DST transition dates.

- Measured against production before merging: `GET /api/symposium/proposals` was returning **51 distinct speaker and organizer email addresses, 43 non-empty private submitter `comments`, and the vote aggregates** (`score`, `voter_count`, `total_votes`) to unauthenticated callers. Zero after. The design decision worth keeping is that the projection is an **allowlist** (`PUBLIC_FIELDS`), not a denylist: a column added to `symposium_proposals` later is withheld until someone lists it, rather than silently published &mdash; which is exactly how the host3&ndash;5 email columns from migration 033 became public in the first place. Ownership became a computed `is_owner` flag; `/api/projects/:slug` and `/api/symposium/sessions/:slug` got the same treatment for `submitted_by`, `admin_notes` and `owner_email`. Reviewed rather than rubber-stamped: the admin-gated `program/edit-proposal.html` prefills six email inputs and still receives them via the privileged branch. Two accepted consequences noted: ownership is tested only on `speaker_email` and `organizer_email` (mirroring the existing PATCH rule), so co-organizers and hosts 3&ndash;5 can no longer read the full record either.

- The migration gives each workshop proposal row an anchor `scheduled_date`/`scheduled_time_utc` derived from its first session, so API consumers stop seeing all five workshops as unscheduled. As submitted it was guarded by `AND scheduled_date IS NULL` &mdash; set once, with nothing to refresh it: nothing in `functions/` writes to `symposium_workshop_sessions`, so a later timing change would leave the anchor silently wrong. Silently is the operative word: `renderCard` guards its single-date line with `if (!isWorkshop)`, so the program page would never reveal the drift and only direct API consumers &mdash; the readers the migration exists for &mdash; would see it. This session is the proof: the Stigmergy Hackathon's first session moved 13:00 to 15:00 hours earlier, and a one-shot run before that would now disagree with the schedule. The guard was dropped so the statement re-derives every anchor from current session rows, making re-running it the documented fix after any timing change; the cost, accepted deliberately, is that a hand-set anchor on a workshop that has session rows will be overwritten, because `symposium_workshop_sessions` is the source of truth. Verified on a SQLite fixture: consecutive runs converge, a run after a session moves re-syncs, talks and session-less workshops are untouched.

- `status.md` recorded `npx wrangler pages deploy .` as the recovery procedure for a stuck deploy, and it was run in Session 42. That command uploads the directory it is handed, filtered **only** by a list hardcoded inside wrangler (`_worker.js`, `_redirects`, `_headers`, `_routes.json`, `functions`, `.DS_Store`, `node_modules`, `.git`, `.wrangler`). It does not read `.gitignore`. The obvious fix, an `.assetsignore` file, was checked against wrangler 4.82.2's source before being written and **would have done nothing**: the Pages path builds its own `IGNORE_LIST`, while the `.assetsignore` reader is reachable only from `buildAssetManifest`/`syncAssets`, the Workers Assets path used by `wrangler deploy`. Shipping it would have left a security control that looks real and is inert. So a root deploy would publish `inbox/` (Luma guest CSVs with attendee emails), `backups/` (D1 member exports) and `.env` to a public CDN. Nothing has leaked &mdash; every such path currently 404s &mdash; the *procedure* was the hazard. `deploy.sh` exports a commit with `git archive` and deploys that, so the upload can contain only tracked files at a known commit: no gitignored path, no stray local file, and no other session's uncommitted work. It additionally refuses to run if anything sensitive is found *tracked*, since `.gitignore` does not retroactively untrack a committed file; that guard matches files rather than directories, because `inbox/.gitignore` is a tracked keep-file present in every export.

- After PR #13 four GET endpoints return different fields to an admin, an owner, a signed-in member and an anonymous visitor. A cache storing one variant and replaying it to a different caller would undo that split in the worst direction. Pages Functions responses are not cached by default, so nothing was broken &mdash; this is a guard against a future `Cache-Control` on the route or a proxy in front of the zone. `functions/_shared/response.js` exports `sessionVaryingJson()`, setting `Vary: Cookie` and `Cache-Control: private, no-store`; the second matters more, since it stops shared caches storing the response at all if `Vary` is ignored or stripped. Centralised rather than inlined so the next endpoint that branches on session has one obvious thing to call. Headers are passed through the init rather than set on a constructed `Response`, so it never depends on the returned response's headers being mutable.

- Two process findings worth recording. **First**, another session worked in this same checkout mid-task, at one point leaving 13 uncommitted files including a half-finished PII fix, and later checking out branches under this session's feet. A `wrangler pages deploy .` at that moment would have published that work; the collision was caught only because `git status` was run before deploying. Everything this session produced was therefore built in throwaway `git worktree`s off `origin/main` and pushed as fast-forwards, leaving the shared tree untouched &mdash; and each change was test-merged against the other session's open branches first. `deploy.sh` closes the deploy half of this permanently by deploying a commit rather than a directory. **Second**, a slow deploy was twice mistaken for a stuck one. `wrangler pages deployment list` puts a *timestamp* in its Status column for past deployments and the word `Active` only on the live one, so a fresh deploy reading &ldquo;just now&rdquo; looks un-deployed; each deployment's own `&lt;hash&gt;.pages.dev` URL returns 404, which reads as a failed upload; and a `curl -I` probe sends HEAD, which never reaches `onRequestGet`, so API response headers appear missing. All three are now documented in `CLAUDE.md`.

---

## Session 51: PDF programme generator, track renumbering, final schedule edits

*2026-09-20*

**Tracks:** content, operations, static-site

- The symposium programme now exists as a 43-page A4 PDF produced by `make_brochure.py`, which reads the schedule live from D1 through `wrangler` and renders print-styled HTML through WeasyPrint. Nothing in it is hand-maintained: there is no fixture file, no copy of the schedule, and no editable intermediate. The correct response to a schedule change is to rerun the script.That choice closes off the obvious alternative of hand-building a deck or document once and patching it as things move. It also creates the failure mode recorded in `status.md`: the published PDF has no trigger behind it, so a D1 edit updates the website instantly and leaves the PDF stale until someone reruns the generator and re-uploads. The R2 key is deliberately stable (`pi-assets/symposium-2026-program.pdf`) so a refresh replaces the file in place and the page link never changes.Two build-time details are load-bearing. The cover artwork is re-encoded to JPEG before embedding, because WeasyPrint embeds WebP losslessly and the New Nature image is high-noise — that one image alone produced a 2.7 MB file; re-encoded it is ~700 KB. Brand fonts are fetched from Google Fonts as TTF using an old User-Agent, which sidesteps needing brotli in WeasyPrint for woff2, and cached in the gitignored `.brochure-cache/`.

- Parallel blocks now publish general talks as Track I and the curated special session as Track II, on both the programme page and the PDF. The underlying `symposium_proposals.schedule_track = 'ii'` values were deliberately left alone.The reason is that `buildTrack2Map()` finds a special session *through* its `'ii'` items — it reads their `session` field to locate the block they run against. Flipping the column would break that lookup and with it the whole parallel-block construction. So the swap is applied at render time in both surfaces instead, which leaves the page's `mainItems` and `track2Items` publishing as the opposite of what their names say. Both files carry a comment documenting the inversion, `renderCard`'s misleading `isTrack2` parameter was renamed, and the trap is written up under Track numbering in `CLAUDE.md`. This is a knowingly accepted naming debt, chosen over a migration that would have required rewriting the pairing logic during symposium week.

- A special session owns a block while its talks are scheduled individually inside it, using the same columns as any other talk. The consequence, unnoticed until now, is that every per-talk time reads as equally decided. That is wrong for a session whose slots exist only to make the block render.The Art of Memory forced the issue: its five talks had no times at all, so the Friday 19:00–21:30 block rendered with everything stacked at 19:00. They were given 30-minute slots in database order, which is arbitrary — the host sets the real order on the day. `running_order_note` carries that caveat, set for `memory` only; NULL means the order is firm.It lives on the session rather than in either renderer because both surfaces need it and they read different sources — the page through `/api/symposium/sessions`, the brochure straight from D1. One limitation is recorded in the migration: the page can only display it inside a *parallel* block, since a special session without a Track II pairing renders as bare cards with no session header to attach it to.

- Every time is now a stack — UTC in bold, then CEST, MYT, PDT and EDT beneath, with `(+1)`/`(−1)` day-boundary markers matching the site's convention. Zone abbreviations are read from each zone for the specific date rather than hardcoded, so they stay correct if the schedule ever crosses a DST boundary; Kuala Lumpur reports a useless `+08` and is overridden to MYT.The full programme prints each parallel track as one continuous run rather than interleaving the two by time slot, headed by a resolved page cross-reference ("For Track II, skip to page 34") built with CSS `target-counter`. Interleaving reads fine on a scrolling web page and badly on paper, where a reader follows one track.Workshops open on a fresh recto, and their schedules were transposed — zones as rows, sessions as columns. A workshop meets four or five times, and a five-line stack per meeting consumed most of a page; transposed it is seven short lines regardless. That alone brought all five workshops onto a single page each, so the abstract and bio trimming that had been planned was not needed and was not done.

- Tim Beiko's talk was retitled from "Marking to Reality" to "Ethereum: A Microcosm for Protocol Studies" with a full abstract replacing a one-line placeholder; the slug was left as `marking-to-reality` since slugs are only user-facing for workshops.Two `co_speakers` fields held the strings `"n/a"` and `"none"`, which were rendering as bylines reading "trent van epps and n/a" *on the live site*, not only in the new PDF — the brochure work surfaced an existing defect. Both cleared in D1, and the generator now filters a placeholder set defensively. Related: name fields are never split on commas or ampersands, because a studio credited as "Paragonday Systems (Norm O'Hagan &amp; Yatú Espinosa)" is one name.c3po PR #15 (four SIG sessions) was reviewed against the standing checklist and merged clean.

- Adding a description to the Protocol Studies Panel was a content request, but the reason it looked wrong was structural, and filling it in would have masked rather than fixed it &mdash; &ldquo;Monstrous Times Magazine Launch&rdquo; still carries no abstract.**The rule cut through the times.** An entry's timezone stack is absolutely positioned so the entry can fragment across a page without dragging the stack with it; the cost is that the stack contributes no height. An entry whose body was shorter than its five zone lines therefore closed its bottom border above its own last conversions. Fixed with a `min-height` on `.entry` equal to the stack.**It jumped to the next page.** `break-after: avoid` on the title block exists to keep a title with the opening of its abstract. With no abstract there is nothing inside the entry to bond to, so the rule bonded the head to the *following* entry and carried both to a fresh page &mdash; leaving the panel alone atop page 21 with page 20 half empty. Entries with no body now opt out via `.entry--bare`.Also: the cover carries a generation date and a pointer to the live schedule, because a PDF is a snapshot and the page is the source of truth; and `byline()` drops to a plain comma list once any single field holds several names, so the now-four-person panel no longer reads &ldquo;X and Y, Z, W&rdquo;.

- `functions/assets/[[path]].js` stamped every `/assets/*` response `max-age=31536000, immutable`. Assets in `pi-assets` are keyed by bare filename and replaced in place &mdash; that is the documented way to add or refresh one &mdash; so the header asserted the opposite of how the bucket is actually used, and any replaced object kept serving its old bytes for a year.It surfaced on the first republish of the programme PDF: `wrangler` reported a successful upload, a direct `r2 object get` returned the new build, and the public URL still served the previous one. Worth recording that four Pages deployments in between did *not* clear it &mdash; these are Function responses and are not tied to the deployment lifecycle the way static files are.Now `max-age=3600, stale-while-revalidate=86400`; note the zone's Browser Cache TTL rewrites that to `max-age=14400` on the way out, so observed freshness is about four hours rather than one. **The change is not retroactive** &mdash; an entry cached under the old header keeps it until it expires &mdash; so the three already-poisoned URLs (the programme PDF, `logo-static.png`, `nn_dates_banner.webp`) were purged by hand from the Cloudflare dashboard, which is zone-level and not reachable from the account or Workers &amp; Pages views. All three verified afterwards as `MISS` carrying the new header, with the bare PDF URL serving a byte-identical copy of the local build.The latent version of this bug was a year old and would have hit anyone replacing the logo; it was only found because this session happened to overwrite an asset and then check the result rather than trusting the upload's success message.

---

## Session 52: Livestream restored mid-event and embedded inline

*2026-09-24*

**Tracks:** static-site, operations

- The stream went down mid-event on 2026-09-23 and was disabled by `148131e`, a session that never wrapped up &mdash; that work exists only in git history, which is why this session is 52 with no 09-23 entry above it. Restoring it is now `LIVESTREAM_BROKEN = false` and nothing else. The original disable left an outage notice in the markup that a future restore had to remember to delete, and the commit message said so in as many words (&ldquo;drop the `.livestream-outage` div&rdquo;) &mdash; a procedure that lives in a commit message is a procedure that will be missed. The notice is now a permanently-present hidden element that `syncLivestream()` reveals, alongside a matching pre-stream note, so the flag is the whole interface in both directions. `LIVESTREAM_GO_LIVE` also moved to `2026-09-24T14:45:00Z`.

- `/embed/live_stream?channel=&lt;channel id&gt;` is the documented way to embed &ldquo;whatever this channel is broadcasting right now&rdquo;, and it is the only form that would need no per-day maintenance. It renders **&ldquo;This video is unavailable&rdquo;**. Verified against `UCcNZ6wTbeeAJ-O_OIhs2j3A` on 2026-09-24 while a broadcast was live and the video's own `playableInEmbed` was `true`, with `/embed/&lt;video id&gt;` for the same broadcast rendering correctly in the same page seconds later &mdash; so this is the endpoint, not the channel, not embedding permissions, and not a transient. Treat it as dead. This closes off the obvious no-maintenance design and forces everything below.

- Because only `/embed/&lt;video id&gt;` works and that id differs for every day of a five-day event, hardcoding it would mean a deploy every morning &mdash; precisely what `LIVESTREAM_GO_LIVE` was introduced to avoid. The new endpoint fetches the channel's `/live` page server-side, reads `isLiveNow` and `videoId` out of the `ytInitialPlayerResponse` blob, and returns `{live, videoId}`. `isLiveNow` is load-bearing: `/live` happily serves the most recent *finished* broadcast when nothing is on, so matching the id alone would embed yesterday's recording as though it were live. Cached at the edge (`s-maxage=120`, `stale-while-revalidate=600`) rather than per-viewer, since during a session every viewer wants the same answer; the page polls it on the same 60s tick as the CTA, so a page left open overnight picks up the next day's broadcast without a reload. **This scrapes a page Google can change without notice, so assume it breaks eventually.** Every failure mode &mdash; fetch error, non-200, consent interstitial, markup change, nothing broadcasting &mdash; returns `{live: false}` and the player is simply not rendered. The &ldquo;Watch Livestream&rdquo; link is static markup and is unaffected either way, so the floor is exactly the behaviour before this session, never a dead box.

- `_headers` has carried a site-wide `Content-Security-Policy` since June whose `frame-src` allowed only `'self'` and `https://calendar.google.com` &mdash; the latter for an embedded Google Calendar that Session 37 removed. Nothing has been embeddable here for months and nobody had reason to notice. **A CSP-blocked iframe renders as a grey box with a broken-image glyph and logs no violation in the page's own console**, so the symptom is indistinguishable from a YouTube-side failure, and the natural next move is to go debug YouTube. What isolated it was a difference in test harness, not in code: the same embed rendered correctly under a plain `python3 -m http.server` and failed under `wrangler pages dev`, because only the latter applies `_headers`. **Local verification of anything header-sensitive has to go through `wrangler pages dev`; a static server silently tests a different site.** `_headers` now lists `https://www.youtube.com` with a comment explaining the failure mode, since there is nowhere else that knowledge would live.

- The iframe ships with no `src` at all and is pointed at YouTube only while `syncLivestream()` says the stream is live, so nothing loads from youtube.com before go-live, after `EVENT_END`, or during an outage &mdash; and a no-JS visitor gets the existing disabled button rather than an empty frame. When the stream ends the `src` is *removed* rather than the element merely hidden, because a hidden iframe with a live src keeps playing audio and holds the YouTube session open. This follows the precedent already set by the button/anchor pair: both ship in markup, JS toggles between them, and a script failure degrades to the honest disabled state. Sizing is `aspect-ratio: 16 / 9` with `max-width: 100%` rather than the padding-top hack, so it tracks the 760px column down to phone width.
